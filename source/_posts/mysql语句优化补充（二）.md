...
![](/assets/articleImg/2019/image-24.png)

## 2、查询条件数据类型一致

为了测试把其中的一个订单id 改为 201903121704170005，看下图不同的查询：
 <!--more--> 
![](/assets/articleImg/2019/image-26-1024x498.png)

显然oid=201903121704170005比 oid='201903121704170005'长很多,因为原来的oid 的字段类型是varchar，而不是数值型。分析一下：

![](/assets/articleImg/2019/image-27-1024x680.png)

类型不一致的第一条，没有走索引。

## 3、不要使用like左模糊查询

![](/assets/articleImg/2019/image-28-1024x258.png)

左模糊不走索引。

![](/assets/articleImg/2019/image-29-1024x426.png)

右模糊不影响索引的使用

![](/assets/articleImg/2019/image-30-1024x597.png)

## 4、select 避免使用\*查询所需字段

![](/assets/articleImg/2019/image-31-1024x850.png)
...
![](/assets/articleImg/2019/image-33-1024x92.png)

但是，并不是所有的查询加上limit 1，都能提高速度，我们再试一次：

![](/assets/articleImg/2019/image-34-1024x189.png)

显然，前者加上limit 1 的查询时间反而比后者没加limit 1 花费更长的时间，即使差距只有0.00006s。那么在什么时候需要一条数据加上limit 1 的时候才能达到优化的效果呢？

请看下面的例子：

![](/assets/articleImg/2019/image-35-1024x462.png)

![](/assets/articleImg/2019/image-36-1024x507.png)

![](/assets/articleImg/2019/image-37-1024x473.png)
...
![](/assets/articleImg/2019/image-38-1024x120.png)
